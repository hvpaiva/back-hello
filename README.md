...
The sample service of the [BACK lab](https://github.com/hvpaiva/back). It's a status page that shows which version is running, in which stage, on which pod, and whether it reaches the bucket and the database the platform gave it.
...
The page is styled as a hang tag: orange in staging, green in production, grey when it runs outside the cluster. The big line is the version, and the barcode is drawn from it, so a new version looks different at a glance. Under it is what the platform gave the service: one hole per replica it declared, a chip for each pod that has answered, the size it asked for with the cpu and memory that came with it, its bucket and its database. When a new version starts answering, the page reloads by itself. You can watch a rollout without touching the browser, and a change to `charts/hello/values-*.yaml` shows up here too.

## How it ships

This repository holds the code and the service's deploy configuration, in `charts/hello/`. The chart that reads it belongs to the platform ([`charts/app`](https://github.com/hvpaiva/back/tree/main/charts/app) in the lab repository), so the service only says what it needs: its name, team, port, size, whether it's public, a bucket (with versioning in production) and a database (medium in production). The platform creates both and hands hello their details.
